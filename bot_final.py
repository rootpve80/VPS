...
# Owner ID (must be set in .env as OWNER_ID). Only this user can add/remove/list admins.
OWNER_ID = int(os.getenv('OWNER_ID', '0'))
# Optional logo url for embed thumbnails
LOGO_URL = os.getenv('LOGO_URL', '')
# Admin IDs (can be set via .env as comma-separated). Default empty; owner should manage admins via bot.
ADMIN_IDS = {int(id_) for id_ in os.getenv('ADMIN_IDS', '').split(',') if id_.strip()}
WATERMARK = " SnowCloud VPS"
...
class HostForgeBot(commands.Bot):
    ...
    async def rotate_presence(self):
        """Rotate bot presence every 5 seconds."""
        await self.wait_until_ready()
        messages = [
            lambda: f"Watching {len(self.db.get_all_vps())}",
            lambda: "powering SnowCloud",
            lambda: "Made by gg!"
        ]
        i = 0
        while not self.is_closed():
            try:
                ...
                await self.change_presence(activity=discord.Activity(type=discord.ActivityType.watching if msg.startswith('Watching') else discord.ActivityType.playing, name=msg))
                i += 1
            except Exception as e:
                logger.error(f"Error rotating presence: {e}")
...
async def on_ready():
    ...
    try:
        await bot.change_presence(activity=discord.Activity(type=discord.ActivityType.watching, name="SnowCloud VPS"))
...
async def show_commands(ctx):
    ...
    try:
        embed = discord.Embed(title="🤖 SnowCloud VPS Bot Commands", color=discord.Color.blue())
...
async def add_admin(ctx, user: discord.User):
    ...
    if OWNER_ID == 0 or ctx.author.id != OWNER_ID:
        await ctx.send("❌ Only the OWNER can add admins. Set OWNER_ID in .env.", ephemeral=True)
        return
...
async def remove_admin(ctx, user: discord.User):
    ...
    if OWNER_ID == 0 or ctx.author.id != OWNER_ID:
        await ctx.send("❌ Only the OWNER can remove admins. Set OWNER_ID in .env.", ephemeral=True)
        return
...
async def list_admins(ctx):
    ...
    if OWNER_ID == 0 or ctx.author.id != OWNER_ID:
        await ctx.send("❌ Only the OWNER can view admin list. Set OWNER_ID in .env.", ephemeral=True)
        return
...
async def create_vps_command(ctx, memory: int, cpu: int, disk: int, owner: discord.Member, 
    ...
    try:
        ...
        try:
            embed = make_embed("🎉 SnowCloud VPS Creation Successful", color=discord.Color.green())
...
async def list_vps(ctx):
    ...
    try:
        ...
        embed = make_embed("Your SnowCloud VPS Instances", color=discord.Color.blue())
...
async def admin_list_vps(ctx):
    ...
    try:
        ...
        embed = make_embed("All SnowCloud VPS Instances", color=discord.Color.blue())
        valid_vps_count = 0
...
async def delete_vps(ctx, vps_id: str):
    ...
    try:
        ...
        await ctx.send(f"✅ SnowCloud VPS {vps_id} has been deleted successfully!")
    except Exception as e:
...
async def connect_vps(ctx, token: str):
    ...
    try:
        ...
        embed = discord.Embed(title="SnowCloud VPS Connection Details", color=discord.Color.blue())
...
async def admin_stats(ctx):
    ...
    try:
        ...
        embed = make_embed("SnowCloud System Statistics", color=discord.Color.blue())
...
async def vps_usage(ctx):
    ...
    try:
        ...
        embed = discord.Embed(title="Your SnowCloud VPS Usage", color=discord.Color.blue())
...
@bot.hybrid_command(name='resources', description='Show host resources: CPU, memory, disk and health')
async def resources(ctx):
    """Show system resource usage and a simple health indicator."""
    try:
        stats = bot.system_stats
        embed = make_embed("SnowCloud Resources", color=discord.Color.blue())
        embed.add_field(name="CPU Usage", value=f"{stats.get('cpu_usage', 0)}%", inline=True)
        embed.add_field(name="Memory Usage", value=f"{stats.get('memory_usage', 0)}% ({stats.get('memory_used', 0):.2f}GB / {stats.get('memory_total', 0):.2f}GB)", inline=True)
        embed.add_field(name="Disk Usage", value=f"{stats.get('disk_usage', 0)}% ({stats.get('disk_used', 0):.2f}GB / {stats.get('disk_total', 0):.2f}GB)", inline=True)
        health = "Good" if stats.get('cpu_usage',0) < 80 and stats.get('memory_usage',0) < 85 and stats.get('disk_usage',0) < 90 else "Degraded"
        embed.add_field(name="Health", value=health, inline=False)
        await ctx.send(embed=embed)
    except Exception as e:
        logger.error(f"Error in resources command: {e}")
        await ctx.send(f"❌ Error fetching resources: {e}", ephemeral=True)
...
async def global_stats(ctx):
    ...
    try:
        ...
        embed = make_embed("SnowCloud Global Usage Statistics", color=discord.Color.blue())
...
async def reinstall_bot(ctx):
    ...
    try:
        await ctx.send("🔄 Reinstalling SnowCloud bot... This may take a few minutes.")
...
"""
        ...
        process = await asyncio.create_subprocess_exec(
            "docker", "build", "-t", "snowcloud-bot", "-f", "Dockerfile.bot", ".",
            stdout=asyncio.subprocess.PIPE,
            stderr=asyncio.subprocess.PIPE
        )
...
class VPSManagementView(ui.View):
    ...
    async def handle_missing_container(self, interaction: discord.Interaction):
        ...
        embed = discord.Embed(title=f"SnowCloud VPS Management - {self.vps_id}", color=discord.Color.red())
    ...
    async def start_vps(self, interaction: discord.Interaction, button: discord.ui.Button):
        try:
            ...
            embed = discord.Embed(title=f"SnowCloud VPS Management - {self.vps_id}", color=discord.Color.green())
            ...
            await interaction.followup.send("✅ SnowCloud VPS started successfully!", ephemeral=True)
        except Exception as e:
    ...
    async def stop_vps(self, interaction: discord.Interaction, button: discord.ui.Button):
        try:
            ...
            embed = discord.Embed(title=f"SnowCloud VPS Management - {self.vps_id}", color=discord.Color.orange())
            ...
            await interaction.message.edit(embed=embed)
            await interaction.followup.send("✅ SnowCloud VPS stopped successfully!", ephemeral=True)
        except Exception as e:
    ...
    async def restart_vps(self, interaction: discord.Interaction, button: discord.ui.Button):
        try:
            ...
            if token:
                ...
                try:
                    ...
                    if ssh_session_line:
                        ...
                        try:
                            owner = await bot.fetch_user(int(vps["created_by"]))
                            embed = discord.Embed(title=f"SnowCloud VPS Restarted - {self.vps_id}", color=discord.Color.blue())
            ...
            await interaction.followup.send("✅ SnowCloud VPS restarted successfully! New SSH details sent to owner.", ephemeral=True)
        except Exception as e:
...
class OSSelectionView(ui.View):
    ...
    async def reinstall_os(self, interaction: discord.Interaction, image: str):
        try:
            ...
            status_msg = await interaction.followup.send("🔄 Reinstalling SnowCloud VPS... This may take a few minutes.", ephemeral=True)
            ...
            try:
                container = bot.docker_client.containers.run(
                    ...
                    detach=True,
                    privileged=True,
                    hostname=f"snowcloud-{self.vps_id}",
                    mem_limit=memory_bytes,
                    cpu_period=100000,
                    ...
                    ports={'22/tcp': str(vps['external_ssh_port'])},
                    volumes={
                        f'snowcloud-{self.vps_id}': {'bind': '/data', 'mode': 'rw'}
                    }
                )
            except docker.errors.ImageNotFound:
            ...
            try:
                ...
                if ssh_session_line:
                    ...
                    try:
                        owner = await bot.fetch_user(int(vps["created_by"]))
                        embed = discord.Embed(title=f"SnowCloud VPS Reinstalled - {self.vps_id}", color=discord.Color.blue())
            ...
            await status_msg.edit(content="✅ SnowCloud VPS reinstalled successfully!")
            ...
            try:
                ...
                try:
                    ...
                    await channel.send(f"❌ Error reinstalling SnowCloud VPS {self.vps_id}: {str(e)}")
                except:
...
class TransferVPSModal(ui.Modal, title='Transfer VPS'):
    ...
    async def on_submit(self, interaction: discord.Interaction):
        try:
            ...
            await interaction.response.send_message(f"✅ SnowCloud VPS {self.vps_id} has been transferred from {old_owner_name} to {new_owner_name}!", ephemeral=True)
            
            try:
                embed = discord.Embed(title="SnowCloud VPS Transferred to You", color=discord.Color.green())
...
async def manage_vps(ctx, vps_id: str):
    ...
    try:
        ...
        embed = discord.Embed(title=f"SnowCloud VPS Management - {vps_id}", color=discord.Color.blue())
...
async def transfer_vps_command(ctx, vps_id: str, new_owner: discord.Member):
    ...
    try:
        ...
        await ctx.send(f"✅ SnowCloud VPS {vps_id} has been transferred from {ctx.author.name} to {new_owner.name}!")
